...
export default class EtchTable extends EtchComponent
{
    update (props, children) {
        super.update(props, children, {update: false})
    ...
    *getColumnTemplates () {
        ...
        for (const child of this[symbols.self].children) {
            ...
            template.props = Object.assign(template.props || {}, {':context': context})
    ...
    onSort (index, comparator, resolver, dir) {
        ...
        let sortAsc = dir
            ? dir === 'asc'
            : element.classList.contains('etch-table-sort-asc')
        ...
        if (!resolver) {
            resolver = (sortColumn) => {
                return sortColumn.dataset.value !== undefined
                    ? sortColumn.dataset.value
                    : sortColumn.innerText.toLowerCase()
            }
        }
    ...
    buildHeader () {
        ...
        for (const template of this.getColumnTemplates()) {
            ...
            const classNames = ['etch-table-cell', template.props.className]
