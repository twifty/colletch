				...
				switch (arg) {
					...
					default:
						...
						if (30 <= arg && arg <= 37) {
							...
							this[symbols.self].foreground = classes[
								arg - 30// Standard Background Colour
							]
						} else if (40 <= arg && arg <= 47) {
							// NOTE: if bold (1) is also present 8 should be added to the index
							this[symbols.self].background = classes[
								arg - 40// Bright Foreground Colour
							]
						} else if (90 <= arg && arg <= 98) {
							this[symbols.self].foreground = palette[
		...
		while (iter) {
			...
			if (parseEscape) {
				...
				if (code === 33 || (code >= 60 && code <= 63)) {
					token.mode =// ';' separated digits are command args
					char
				} else if (code >= 48 && code <= 57) {
					token.currArg +=// argument separator
					char
				} else if (code === 59) {
					if (token.currArg) {
					...
					token.modifier =// The command itself
					char
				} else if (code >= 64 && code <= 126) {
					token.command = char
					parseEscape =// Illegal char within escape sequence
					false
				} else {
